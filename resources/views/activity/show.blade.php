            ...
            <div class="w-2/3 pl-16 lg:pl-64">
                ...
                <div class="text-white w-[130%] mb-8 flex flex-col">
                    <p class="mb-2 flex items-center">
                        <span class="text-[#f5af00] pr-1"><x-location-icon></x-location-icon></span> {{$activity->location}}
                    </p>

                    <p class="mb-2 flex items-center">
                        <span class="fill-[#f5af00] pr-1"><x-food-icon></x-food-icon></span> {{$activity->food ? "Ja" : "Nee"}}
                    </p>

                    <p class="mb-2 flex items-center">
                        <span class="text-[#f5af00] pr-1"><x-money-icon></x-money-icon></span>
                        €{{$activity->price}}
                    </p>
                    ...
                    @if(date('d-m-Y', strtotime($activity->start_date)) == date('d-m-Y', strtotime($activity->end_date)))
                        <p class="mb-2 flex items-center"><span class="text-[#f5af00] pr-1"><x-clock-icon></x-clock-icon></span> {{ date('d-F-Y \o\m H:i', strtotime($activity->start_date)) }}
                            - {{ date('H:i', strtotime($activity->end_date)) }}</p>
                    @else
                        <p class="mb-2 flex items-center"><span class="text-[#f5af00] pr-1"><x-clock-icon></x-clock-icon></span> {{ date('d-F-Y h:i', strtotime($activity->start_date)) }}
                            tot {{ date('d-F-Y h:i', strtotime($activity->end_date)) }}</p>
                    @endif

                    <p class="mb-2 flex items-center">
                        <span class="text-[#f5af00] pr-1"><x-group-icon></x-group-icon></span>
                        {{$activity->participants->count()}}
                    </p>

                    @isset($activity->needs)
                        <p class="mb-2 flex items-center">
                            <span class="text-[#f5af00] pr-1"><x-shopping-card-icon></x-shopping-card-icon></span>
                            {{$activity->needs}}
                        </p>
                    @endisset
