<a href="{{ route('activity.show', $activity) }}" class="w-[350px] bg-white transition-all ease-in-out" id="card" name="card">
    <div class="w-full h-[350px] bg-center flex justify-center items-center group overflow-hidden bg-cover" style="background-image: url({{ file_exists(public_path('storage/' . $activity->image)) ? asset("storage/" . $activity->image) : $activity->image }})">
